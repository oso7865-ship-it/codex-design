...
const deliveryAddress = computed(() => {
  return appStore.addresses.find(
    (address) => address.name === appStore.selectedDelivery.addressName,
  )
})
</script>
...
<template>
  <div class="page management-page profile-page">
    ...
    <section class="page-intro">
      ...
      <h1>배송 상세</h1>
      <p>배송 번호 {{ appStore.selectedDelivery.id }}</p>
    </section>

    <section class="detail-card">
      <div class="detail-card__heading">
        <strong>{{ appStore.selectedDelivery.addressName }} 배송</strong>
        <StatusBadge :status="appStore.selectedDelivery.status" />
      </div>
      <dl class="detail-list">
        <div>
          <dt>배송 예정일</dt>
          <dd>{{ appStore.selectedDelivery.deliveryDate }}</dd>
        </div>
        <div>
          <dt>메뉴 수량</dt>
          <dd>{{ appStore.selectedDelivery.menuCount }}개</dd>
        </div>
        <div>
          <dt>받는 분</dt>
    ...
    <aside class="notice-box notice-box--info">
      <div>
        ...
        <p>배송이 시작되면 배송사와 운송장 번호를 이곳에서 확인할 수 있습니다.</p>
      </div>
    </aside>
  </div>
</template>
